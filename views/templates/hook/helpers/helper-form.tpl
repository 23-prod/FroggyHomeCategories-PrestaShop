 ...
 *}
        ...
        {foreach from=$froggyhelper.configuration.form key=form_key item=form}
            ...
            {foreach from=$form.fields item=field}
				<label for="{$field.name|escape:'html':'UTF-8'}"> {$field.label|escape:'html':'UTF-8'}</label>
                    ...
                    <div>
                        {if $field.type eq 'radio'}
                            <span class="switch prestashop-switch fixed-width-lg">
                                    ...
                                    <label for="{$field.name|escape:'html':'UTF-8'}_on"> {l s='Yes' mod='froggyhomecategories'}</label>
                                    <input type="radio" name="{$field.name}" id="{$field.name}_off" value="0" {if !isset($field.value) || $field.value ne 1}checked="checked"{/if} />
                                    <label for="{$field.name|escape:'html':'UTF-8'}_off"> {l s='No' mod='froggyhomecategories'}</label>
		...
		<div class="margin-form">
			<button class="button" name="{$froggyhelper.module_name|escape:'html':'UTF-8'}-submit" id="{$froggyhelper.module_name|escape:'html':'UTF-8'}-form-submit-btn" value="1" type="submit">
				 {l s='Save' mod='froggyhomecategories'}
			</button>
		</div>
